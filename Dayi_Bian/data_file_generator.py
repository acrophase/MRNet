...
for item in enumerate(data.keys()):
    ...
    acc_y_arr = scaler.fit_transform(acc_y_arr.reshape(len(acc_y_arr) , len(acc_y_arr[0])))
    acc_z_arr = scaler.fit_transform(acc_z_arr.reshape(len(acc_z_arr) , len(acc_z_arr[0])))

    windowed_inp = np.concatenate((np.expand_dims(edr_hrv, 1), np.expand_dims(edr_rpeak, 1), np.expand_dims(adr, 1)), axis = 1)
    windowed_raw_sig = np.concatenate((np.expand_dims(ecg_arr, 1), np.expand_dims(acc_y_arr, 1), np.expand_dims(acc_z_arr, 1)), axis = 1)
