...
def extract_data(path, srate, window_length):
    ...
    for sub_id in tqdm(subjects):
        ...
        subpath_activity = pd.read_csv(
            os.path.join(path, sub_id, sub_id + "_activity.csv")
        )
        subpath_activity = subpath_activity.rename(
        ...
        for index in range(1, len(subpath_activity)):
            if index == 1:
                annotation_per_window = [
                    subpath_activity.iloc[index - 1, 2]
                    for i in range(
                        int(round(subpath_activity.iloc[index, 1] / seconds_per_window))
                    )
                ]
                prev = (
                    round(subpath_activity.iloc[index, 1] / seconds_per_window)
                    * seconds_per_window
                )
            else:
                annotation_per_window += [
                    ...
                    for i in range(
                        int(
                            round(
                                (subpath_activity.iloc[index, 1] - prev)
                                / seconds_per_window
                            )
                        )
                    )
        ...
        for i in range(len_parameter):
            ...
            windowed_acc_x.append(
                acc_x_axis[i * window_length : (i + 1) * window_length]
            )
            windowed_acc_y.append(
                acc_y_axis[i * window_length : (i + 1) * window_length]
            )
            windowed_acc_z.append(
                acc_z_axis[i * window_length : (i + 1) * window_length]
            )
            for item in rpeaks:
        ...
        baseline_ecg = [
            windowed_ecg[item]
            for item in list(np.where(np.array(annotation_per_window) == 0)[0])
        ]
        stairs_ecg = [
            windowed_ecg[item]
            for item in list(np.where(np.array(annotation_per_window) == 7)[0])
        ]
        soccer_ecg = [
            windowed_ecg[item]
            for item in list(np.where(np.array(annotation_per_window) == 6)[0])
        ]
        cycling_ecg = [
            windowed_ecg[item]
            for item in list(np.where(np.array(annotation_per_window) == 2)[0])
        ]
        driving_ecg = [
            windowed_ecg[item]
            for item in list(np.where(np.array(annotation_per_window) == 3)[0])
        ]
        lunch_ecg = [
            windowed_ecg[item]
            for item in list(np.where(np.array(annotation_per_window) == 4)[0])
        ]
        walking_ecg = [
            windowed_ecg[item]
            for item in list(np.where(np.array(annotation_per_window) == 8)[0])
        ]
        working_ecg = [
            windowed_ecg[item]
            for item in list(np.where(np.array(annotation_per_window) == 9)[0])
        ]
        ...
        data.update(
            {
                sub_id: {
                    "ECG": {
                        "ECG_DATA": windowed_ecg,
                        "RPEAKS": RPEAKS,
                        "AMPLITUDES": amplitudes,
                        "BASELINE_ECG": baseline_ecg,
                        "STAIRS_ECG": stairs_ecg,
                        "SOCCER_ECG": soccer_ecg,
                        "CYCLING_ECG": cycling_ecg,
                        "DRIVING_ECG": driving_ecg,
                        "LUNCH_ECG": lunch_ecg,
                        "WALKING_ECG": walking_ecg,
                        "WORKING_ECG": working_ecg,
                        "BASELINE_RPEAKS": baseline_rpeaks,
                        "STAIRS_RPEAKS": stairs_rpeaks,
                        "SOCCER_RPEAKS": soccer_rpeaks,
                        "CYCLING_RPEAKS": cycling_resp,
                        "DRIVING_RPEAKS": driving_rpeaks,
                        "LUNCH_RPEAKS": lunch_rpeaks,
                        "WALKING_RPEAKS": walking_rpeaks,
                        "WORKING_RPEAKS": working_rpeaks,
                        "BASELINE_AMPS": baseline_amplitudes,
                        "STAIRS_AMPS": stairs_amplitudes,
                        "SOCCER_AMPS": soccer_amplitudes,
                        "CYCLING_AMPS": cycling_amplitudes,
                        "DRIVING_AMPS": driving_amplitudes,
                        "LUNCH_AMPS": lunch_amplitudes,
                        "WALKING_AMPS": walking_amplitudes,
                        "WORKING_RPEAKS": working_amplitudes,
                    },
                    "ACC": {
                        "ACC_DATA": windowed_acc,
                        "ACC_X": windowed_acc_x,
                        "ACC_Y": windowed_acc_y,
                        "ACC_Z": windowed_acc_z,
                        "BASELINE_ACC": baseline_acc,
                        "STAIRS_ACC": stairs_acc,
                        "SOCCER_ACC": soccer_acc,
                        "CYCLING_ACC": cycling_acc,
                        "DRIVING_ACC": driving_acc,
                        "LUNCH_ACC": lunch_acc,
                        "WALKING_ACC": walking_acc,
                        "WORKING_ACC": working_acc,
                    },
                    "RESP": {
                        "RESP_DATA": windowed_resp,
                        "BASELINE_RESP": baseline_resp,
                        "STAIRS_RESP": stairs_resp,
                        "SOCCER_RESP": soccer_resp,
                        "CYCLING_RESP": cycling_resp,
                        "DRIVING_RESP": driving_resp,
                        "LUNCH_RESP": lunch_resp,
                        "WALKING_RESP": walking_resp,
                        "WORKING_RESP": working_resp,
                    },
