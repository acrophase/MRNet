...
def edr_adr_extraction(acc, rpeaks, rpeak_amplitudes, interp_rate=4, ds_factor=10):
    ...
    for item in rpeaks:
        ...
        interpolated_signal = (
            interpolated_signal - np.mean(interpolated_signal)
        ) / np.std(interpolated_signal)
    ...
    for item in rpeak_amplitudes:
        ...
        interpolated_signal_rp = (
            interpolated_signal_rp - np.mean(interpolated_signal_rp)
        ) / np.std(interpolated_signal_rp)
        final_edr_rpeak.append(
            scipy.signal.filtfilt(fbpB, fbpA, interpolated_signal_rp)
        )
        i += 1
